  ...
  return (
    <div className="flex flex-col items-center justify-center py-12">
      <div className="bg-white rounded-3xl border-8 border-slate-800 shadow-2xl p-8 flex flex-col items-center w-full max-w-sm">
        ...
        <h2 className="text-2xl font-black text-slate-800 uppercase italic mb-1">Coach Only</h2>
        <p className="font-bold text-slate-500 mb-6" dir="rtl">אזור הורים — הקישו קוד</p>
        ...
        {locked ? (
          <p className="font-black text-red-500 text-lg mb-4" dir="rtl">נעול! נסו שוב בעוד {lockedSecs} שניות</p>
        ) : tries > 0 ? (
          <p className="font-black text-red-500 mb-4" dir="rtl">קוד שגוי ({tries}/{config.pinLockoutTries})</p>
        ) : null}
  ...
  return (
    <div className="space-y-6 anim-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <MetricCard label="Win Rate" value={`${stats.winRate}%`} icon={<div className="w-12 h-12 rounded-full border-8 border-slate-100 border-t-green-500 border-r-green-500 rotate-45"></div>} />
        <MetricCard label="Avg Time / Question" value={`${stats.avgTime}s`} icon={<Zap className="text-yellow-400 fill-yellow-200 h-10 w-10" />} />
        <MetricCard label="Total Wins" value={stats.wins} icon={<Trophy className="text-blue-500 fill-blue-200 h-10 w-10" />} />
      </div>

      {/* PER-SUBJECT ACCURACY */}
      <div className="bg-white border-4 border-slate-200 rounded-3xl shadow-lg p-6">
        <h3 className="text-lg font-black text-slate-600 uppercase tracking-wide mb-4">Accuracy by Subject</h3>
        {Object.keys(stats.bySubject).length === 0 ? (
          <p className="font-bold text-slate-400" dir="rtl">עדיין אין משחקים — שלחו את תומי לשחק!</p>
        ) : (
        ...
        <div className="overflow-x-auto max-h-96 overflow-y-auto">
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-slate-50">
              <tr>
                <Th>Time</Th><Th>Event</Th><Th>Result</Th><Th>Score</Th><Th>Coins</Th>
              </tr>
            </thead>
            <tbody className="divide-y-2 divide-slate-100 font-bold">
              {state.battleLog.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-6 text-slate-400 font-bold text-center">No matches yet</td></tr>
              )}
      ...
      <div className="bg-white border-4 border-slate-200 rounded-3xl shadow-lg p-6">
        <h3 className="text-lg font-black text-slate-600 uppercase tracking-wide mb-4 flex items-center gap-2">
          <ShoppingBag size={20} /> Purchases to Fulfill
        </h3>
        {state.purchases.length === 0 ? (
          <p className="font-bold text-slate-400">No purchases yet</p>
        ) : (
